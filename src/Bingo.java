...
public class Bingo {
	...
	public static void main(String[] args) {
		...
		System.out.println("Benvenuto/a al Bingo");
		String continua;
		do {
			System.out.print("Quante schede vuoi? (1 - 6) "); //scelta n schede
			...
			boolean bingo = false;
			boolean[] cinquina = new boolean[nSchedePrese]; //booleane per check vittoria

			// inizio estrazione numeri
			while (!bingo) {
				estratto = pallottoliere[r.nextInt(90)];
				while (nEstratti.contains(estratto)) {
					estratto = pallottoliere[r.nextInt(90)];
				}
				...
				for (int s = 0; s < nSchedePrese; s++) {
					for (int i = 0; i < 3; i++) {
						for (int j = 0; j < 5; j++) {
							if (schede[s][i][j] == estratto) {
								schede[s][i][j] = -estratto; // Segna il numero come estratto con segno negativo
